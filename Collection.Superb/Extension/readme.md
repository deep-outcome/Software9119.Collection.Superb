...
- `IEnumerableExtension`
    ...
    - <strong style="background-color:rgba(186 246 226 / 0.63)"><u>`AsOrTo` or `Into` for chosen [`System.Collections.Concurrent` Namespace](https://learn.microsoft.com/en-us/dotnet/api/system.collections.concurrent?view=net-10.0) types</u></strong>
        ...
        ```
        - `BlockingCollection<Item>? AsOrToBlockingCollection<Item>(IEnumerable<Item>?, int?, BlockingCollectionType, EnumerableNullBehavior)` – casts `IEnumerable<T>` into blocking collection, or casts or copies it into intermediate `IProducerConsumerCollection<T>` before wrapping to blocking collection
