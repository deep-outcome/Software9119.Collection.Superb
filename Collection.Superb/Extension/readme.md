...
- `IEnumerableExtension`
    ...
    - <strong style="background-color:rgba(186 246 226 / 0.63)"><u>`AsOrTo` or `Into` for chosen [`System.Collections.Concurrent` Namespace](https://learn.microsoft.com/en-us/dotnet/api/system.collections.concurrent?view=net-10.0) types</u></strong>
        - `ConcurrentBag<Item>? AsOrToConcurrentBag<Item>(IEnumerable<Item>?, EnumerableNullBehavior)` – casts or copies `IEnumerable<T>` into concurrent bag
        ...
        - `OrderablePartitioner<Item>? AsOrderablePartitioner<Item>(IEnumerable<Item>?, bool, EnumerablePartitionerOptions, EnumerableNullBehavior)` – wraps `IEnumerable<T>` into orderable partitioner